...
namespace Ad.Util
{
    ...
    public static class StringHelper
    {
        ...
        /// <summary>
        /// 从被某字符分割字符串中移除字符（如：1,43,43,3, 中移除3， 后= 1,43,43）
        /// </summary>
        /// <param name="value">原字符</param>
        /// <param name="split">分隔符</param>
        /// <param name="removeValue">移除的字符</param>
        /// <returns></returns>
        public static string RemoveStrFromSplitString(string value, char split, string removeValue)
        {
            ...
            return string.Join(split.ToString(), arr.Where(f => f != removeValue).Select(f => f).ToArray());
        }
        ...
        public static string GenerateString(string pre, string str, int no, int len,char x)
        {
            int noLen = no.ToString().Length;
            string suffixStr = no.ToString();
            for (int i = noLen; i < len; i++)
            {
                suffixStr = x + suffixStr;
            }
            if(null == pre){
                pre = "";
            }
            return pre + str + suffixStr;
        }

        // 默认添加后面
        public static string AddCharToString(int len, char x, string original, bool isBehind = true)
        {
            if (len < 0)
            {
                return original;
