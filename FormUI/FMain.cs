...
namespace FormUI
{
    public partial class FMain : Form
    {
        ...
        private void OtherContextMenuItemSelected(object sender, EventArgs e)
        {
            ...
            else if (toolItem.Name == Ad.Util.Const.OtherContextMenuPrefix + (int)EnumHelper.OtherContextMenuEnum.CallExtension)
            {
                CallExtensionFunc();
            }
        }
        ...
        {
            this.axCTISrv.cmdCallHold(AnswerHelper.CurrentChannelId);
            this.timeCallDivert.Enabled = true;
        }

         // 呼叫转移定时器
        private void timeCallDivert_Tick(object sender, EventArgs e)
        {
            this.axCTISrv.cmdCallDivert(AnswerHelper.CurrentChannelId);
            this.timeCallDivert.Enabled = false;
            this.timerDial.Enabled = true;
        }

        // 拨号定时器
        private void timerDial_Tick(object sender, EventArgs e)
        {
            try
            {
            ...
            finally { this.timerDial.Enabled = false; }
        ...
        private void ForceInsertFunc()
        {
            try
            {
                this.axCTISrv.cmdDial(AnswerHelper.CurrentChannelId, AnswerHelper.SelectedTelControl.LblSeatText + AppSetting.ForcedInsert);
            }
            catch (Exception) { }
        ...
        private void MonitorFunc()
        {
            try
            {
                this.axCTISrv.cmdDial(AnswerHelper.CurrentChannelId, AnswerHelper.SelectedTelControl.LblSeatText + AppSetting.Monitor);
            }
            catch (Exception) { }
        }
        ...
        {
            this.axCTISrv.cmdDial(AnswerHelper.CurrentChannelId, AnswerHelper.SelectedTelControl.LblSeatText + AppSetting.ForcedHangUp);
        }

        private void CallExtensionFunc()
        {
            try
            {
                this.axCTISrv.cmdDial(AnswerHelper.CurrentChannelId, AnswerHelper.SelectedTelControl.LblSeatText);
        ...
        private void tabControl1_SelectedIndexChanged(object sender, EventArgs e)
        {
            switch (this.tabControl1.SelectedIndex)
            {
                ...
                case (int)EnumHelper.FunctionEnum.KnowledgeBase:
                    if (!StaticStateHelper.IsInitKnowledgeBase.GetValueOrDefault())
                    {
                        this.InitKnowledgeBase();
                        StaticStateHelper.IsInitKnowledgeBase = true;
                    }
                    break;
        ...
        public void setRtbCallRemark(string remark)
        {
            this.rtbCallRemark.Text += remark + Environment.NewLine;
        }
        #endregion
        ...
        private void InitSearchModel()
        {
            if (!LoginHelper.IsGodUser)
            {
                try
                {
                    if (LoginHelper.FunIsHasOper(EnumHelper.FunctionEnum.Search, EnumHelper.OperationEnum.Add))
                    {
        ...
        private void InitKnowledgeBase()
        {
            if (!LoginHelper.IsGodUser)
            {
                try
                {
                    if (!LoginHelper.FunIsHasOper(EnumHelper.FunctionEnum.KnowledgeBase, EnumHelper.OperationEnum.Add))
                    {
                        this.btnKnowledgeAdd.Visible = false;
                    }
                    if (!LoginHelper.FunIsHasOper(EnumHelper.FunctionEnum.KnowledgeBase, EnumHelper.OperationEnum.Update))
                    {
                        this.dgvKnowledge.Columns["Know_Update"].Visible = false;
                    }
                    if (!LoginHelper.FunIsHasOper(EnumHelper.FunctionEnum.KnowledgeBase, EnumHelper.OperationEnum.Delete))
                    {
                        this.dgvKnowledge.Columns["Know_Delete"].Visible = false;
                    }
                }
                catch (Exception) { }
            }
        ...
        private void axCTISrv_onCall(object sender, AxwpCTIOcx.__CTISrv_onCallEvent e)
        {
            ...
            try
            {
                if (controls != null && controls.Length > 0)
                {
                    ...
                    FCallModule fCallModule = new FCallModule(e.assPhone);
                    fCallModule.Owner = this;
                    fCallModule.ShowDialog();

                    //var channelEnumList = AnswerHelper.GetAllEnableChannels().Where(m => m.dstID == e.iD);
                    //if (channelEnumList.Any())
                    //{
                    //    UserChannel userChannel = channelEnumList.First();
                    //    FCallModule fCallModule = new FCallModule(userChannel.dstPhone, userChannel.iD);
                    //    fCallModule.Owner = this;
                    //    fCallModule.ShowDialog();
                    //}
                }
        ...
        private void axCTISrv_connected(object sender, AxwpCTIOcx.__CTISrv_connectedEvent e)
        {
            ...
            if (AnswerHelper.BombScreen == null)
            {
                if (!string.IsNullOrWhiteSpace(e.assPhone))
                {
                    AnswerHelper.BombScreen = new FBombScreen(e.assPhone, e.iD);
                    AnswerHelper.BombScreen.Owner = this;
                    AnswerHelper.BombScreen.ShowDialog();
                }
            }
        }
