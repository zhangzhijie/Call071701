...
namespace FormUI
{
    public partial class FCallModule : Form
    {
        ...
        public FCallModule(string phone)
        {
            this.phone = phone;
        ...
        private void btnCallDetailSave_Click(object sender, EventArgs e)
        {
            ...
            {
                string remark = "拨出时间： " + entity.EntryDate.Value.ToString("yyyy/MM/dd HH:mm:ss")+"    ";
                remark += "电话号:    " + Ad.Util.StringHelper.AddCharToString(20, ' ', phoneStr)+"    ";
                remark += "客户名称： " + this.txtCustName.Text.Trim();
                ((FMain)Owner).setRtbCallRemark(remark);
                this.Dispose();
            }
        }
    }
